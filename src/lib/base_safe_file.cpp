...
// tables of precalculated CRC values
static uint32_t __crc32_table__[256] = {
...
/*
 * calculate CRC of block of data (memory)
 * @param data pointer to data
 * @param len length of data
 *
 * @return CRC of data
 */
tcrc_t CBaseSafeFile::crc32 ( void *data, int32_t dlen ) 
{
    uint8_t *p = NULL;

    C_LOG_DEBUG("CRC OFFSET:%d LEN:%d", (int)((uint8_t *)data-work_area), dlen);
    uint32_t result = 0xffffffff;
    crc_zero_flag = 1;

    for (p = (uint8_t *)data; dlen > 0; ++p, --dlen) 
    {
        if (crc_zero_flag && *p) crc_zero_flag = 0; 
        result = (result << 8) ^ __crc32_table__[(result >> 24) ^ *p];
    }

    C_LOG_DEBUG("CRC %08X", result);
    return result;
}
